...
class PremixController extends Controller
{
    ...
    public function index()
    {
        ...
        return Inertia::render('Admin/Premixes/Premix', [
            'premixes' => Premix::with(['premixingredients.rawmaterial'])->get(),
        ]);
    }
    ...
    public function replenish(Request $request)
    {
        ...
        foreach ($premix->premixingredients as $ingredient) {
            ...
            if ((($stock - floor($stock)) * $ingredient->rawmaterial->typeQuantity) > 1) {
                $nextStock = ($stock - floor($stock)) * $ingredient->rawmaterial->typeQuantity;
