    ...
    <AuthenticatedLayout>
        <article class="flex-1 w-full p-5 overflow-auto bg-white mx-5 rounded">
            ...
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold">Inventory Reports</h2>
                ...
                <table class="w-full text-sm text-left">
                    <thead class="text-xs uppercase">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">Used For.</th>
                    ...
                    <tbody>
                        <tr v-for="product in products" :key="product.id" class="border-t border-gray-700">
                            <td class="px-6 py-4">{{ product.productName }}</td>
                            <td class="px-6 py-4">{{ product.category }}</td>
                            <td class="px-6 py-4">₱{{ product.price }}</td>
                            <td class="px-6 py-4 flex items-center space-x-3">
                                <Link :href="route('products.edit', product.id)" class="text-blue-400 hover:text-blue-600">
