...
<template>
    <div class="h-screen w-screen p-5 bg-[#D9D9D9]">
        <div class="flex h-full w-full gap-5">
            <!-- Sidebar -->
            <aside
                :class="isSidebarOpen ? 'w-[20%] min-w-[17rem]' : 'w-[6.5%] 2xl:w-[5%] xl:visible xl:block hidden pt-10'"
            ...
            >
                ...
                <div class="overflow-y-auto">
                    ...
                    <ResponsiveNavLink
                        ...
                        :active="route().current('deliveries.index') || route().current('deliveries.create') || route().current('deliveries.edit') || route().current('deliveries.show')"
                        class="flex ps-3 gap-2 items-center mb-1 text-sm"
                    >
                        <div>
                    ...
                    <Dropdown2nd v-if="hasRole('admin')">
                        <template #trigger>
                            <button
                            type="button"
                            class="w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out"
                            :class="['w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out',
                            route().current('products.index') || route().current('products.create') || route().current('products.edit') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            route().current('premixes.index') || route().current('premixes.create') || route().current('premixes.edit') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            route().current('rawmaterials.index') || route().current('rawmaterials.create') || route().current('rawmaterials.edit') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            route().current('transfers.index') || route().current('transfers.create') || route().current('transfers.show') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : ''
                            ]">
                                <div class="flex items-center">
                                    <svg @click="toggleSidebar" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-layers mr-2" viewBox="0 0 16 16">
                        ...
                        <template #content>
                            <div class="px-6" v-if="isSidebarOpen">
                                ...
                                <ResponsiveNavLink
                                    ...
                                    :active="route().current('rawmaterials.index') || route().current('rawmaterials.create') || route().current('rawmaterials.edit')"
                                    class="flex ps-3 gap-2 items-center mb-1 text-sm"
                                >
                                ...
                                <ResponsiveNavLink
                                    ...
                                    :active="route().current('transfers.index') || route().current('transfers.create') || route().current('transfers.show')"
                                    class="flex ps-3 gap-2 items-center mb-1 text-sm"
                                >
                    ...
                    <Dropdown2nd v-if="hasRole('admin')">
                        <template #trigger>
                            ...
                            class="w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out"
                            :class="['w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out',
                            route().current('clients.index') || route().current('clients.create') || route().current('clients.edit') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            route().current('suppliers.index') || route().current('suppliers.create') || route().current('suppliers.edit') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            ]">
                    ...
                    <Dropdown2nd v-if="hasRole('admin')">
                        <template #trigger>
                            ...
                            class="w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out"
                            :class="['w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out',
                            route().current('purchases.index') || route().current('purchases.create') || route().current('purchases.show') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            ]">
                                <div @click="toggleSidebar" class="flex items-center">
                        ...
                        <template #content>
                            <div class="px-6" v-if="isSidebarOpen">
                                ...
                                <ResponsiveNavLink
                                    ...
                                    :active="route().current('purchases.index') || route().current('purchases.create') || route().current('purchases.show')"
                                    class="flex ps-3 gap-2 items-center mb-1 text-sm"
                                >
                    ...
                    <Dropdown2nd v-if="hasRole('admin')">
                        <template #trigger>
                            ...
                            class="w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out"
                            :class="['w-full my-1 flex justify-between items-center text-sm ps-3 px-3 gap-2 block w-full py-2 rounded-lg hover:text-white hover:bg-[#0108EE] hover:shadow-xl hover:rounded-lg transition duration-150 ease-in-out',
                            route().current('salesreports.index') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            route().current('inventoryreports.index') ? 'block w-full py-2 text-white bg-[#0108EE] shadow-xl rounded-lg transition duration-150 ease-in-out' : '',
                            ]">
                                <div @click="toggleSidebar" class="flex items-center">
                            ...
                            <Dropdown>
                                <template #trigger>
                                    <button class="rounded-full p-2 transition-all duration-200 hover:bg-gray-100 hover:border-black focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        ...
                                        <!-- Notification Badge -->
                                        <span v-if="$page.props.products && $page.props.products.length > 0" class="absolute top-0 right-0 inline-flex items-center justify-center w-4 h-4 text-xs font-medium text-white bg-[#F7C849] border-2 border-white rounded-full" />
                                    </button>
                                </template>
                                ...
                                <template #content 
                                    ...
                                    <div class="px-4 py-2">
                                        ...
                                        <div class="max-h-60 overflow-y-auto">
                                            <div v-if="$page.props.products && $page.props.products.length > 0">
                                                <div v-for="product in $page.props.products" :key="product.productID">
                                                    <div class="flex items-center gap-2 py-2 px-1 hover:bg-gray-100 rounded-lg cursor-pointer"
                                                    v-if="product.quantity <= product.criticalLevel"
                                                    >
                                                        <Link :href="route('products.edit', product.productID)" class="text-[#0109F4] hover:text-blue-400">
                                                            <div class="flex-shrink-0">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#EF4444" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p class="text-xs text-gray-500">
                                                                    Current quantity: {{ product.criticalLevel }}
                                                                </p>
                                                            </div>
                                                        </Link>
                                                    </div>
